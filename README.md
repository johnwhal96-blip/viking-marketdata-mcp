...
Он предоставляет 45 инструментов:
...
Состояние робота как процесса, а не его портфелей: жив ли он, какая на нём
версия, перезапускался ли он, обновится ли версия при следующем рестарте.
Раньше на эти вопросы через MCP ответить было нельзя — приходилось искать
в журнале строку `Robot was started with version …`, а она появляется только
постфактум, после очередного старта.
...
на равенство объявила бы такой робот устаревшим. `restart_updates_version` —
обратное значение: `true` означает, что ближайший рестарт сменит версию робота.
