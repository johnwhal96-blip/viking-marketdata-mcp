...
Он предоставляет 46 инструментов:
...
- `get_messages_history` — неподавляемые сообщения платформы (вкладка Notifications):
  плановые перезапуски роботов, объявления;
- `get_robot_status` — состояние самого робота: подключение, версия, счётчик
  главного цикла, время робота;
...
Неподавляемые сообщения платформы — то, что интерфейс робота показывает во
вкладке Notifications: плановые перезапуски роботов, объявления. В логи робота
(`get_robot_log_history`) эти сообщения не попадают, поэтому вопрос «что будет
с роботом» отвечается только этим инструментом.

Вызывает `messages.get_history` (api.md, раздел 11.13.4). Сообщения относятся
к учётной записи, а не к роботу или портфелю: параметров `robot_id` и
`portfolio` нет.

Параметры:

- `date_from` и `date_to` в ISO 8601 с обязательным часовым поясом — границы
  включительные и преобразуются в `mint`/`maxt` формата `epoch_msec`;
- `include_read` — по умолчанию `false`: Viking отдаёт только непрочитанные;
  `true` добавляет прочитанные (`read=true` в запросе);
- `limit` от 1 до 100, по умолчанию 100 — ограничение самого API;
- `timezone` — для поля `dt_iso`, по умолчанию `Europe/Moscow`.

Ответ сохраняет все поля сообщения. `msg` — текст и одновременно уникальный
ключ сообщения; `st` дублируется читаемым `state` (`unread`/`read`); `dt` —
`epoch_msec` (не `epoch_nsec`, как в логах) и дополняется `dt_iso`.
`count_in_database` — счётчик Viking, если он пришёл в ответе.

### `get_robot_status`

Текущее состояние робота как процесса, а не его портфелей: жив ли он, какая
на нём версия и какая сборка лежит на сервере. Раньше даже текущую версию
через MCP получить было нельзя — приходилось искать в журнале строку
`Robot was started with version …`, а она относится к прошедшему старту.

Это снимок «сейчас». Факт и время перезапуска по нему не определяются:
для них остаются `get_robot_log_history` и `get_messages_history`.

Данные берутся из того же `robot.subscribe`, что и статус торговли портфелей:
второго запроса к Viking нет. `value.re` остаётся за
`get_robot_portfolio_trading_status`, всё остальное из `value` отдаёт этот
инструмент — включая поля, которые платформа может добавить позже.

Параметры:

- `robot_id`;
- `timezone` — для полей `dt_iso`, `rvd_iso`, `svd_iso`, по умолчанию `Europe/Moscow`;
- `raw` — вернуть `value` без читаемых надстроек.

Ответ сохраняет все исходные поля и добавляет читаемые: `connected` (`rc`),
`process` (`ps`), `trading` (`tr`), `market_data_status` (`mdc`),
`transaction_status` (`trc`), `main_loop_counter` (`mc`), `robot_version` (`rv`),
`server_version` (`sv`), `dt_iso` (`dt` в `epoch_msec`), `rvd_iso` и `svd_iso`
(`epoch_sec`, `-1` означает «неизвестно» и даёт `null`).

`same_build` сравнивает `rv` и `sv` **по общему префиксу, а не на равенство**:
Viking обрезает эти строки до разной длины для одной и той же сборки — в примере
официальной документации `rv` равен `"ec1d046c"`, а `sv` — `"ec1d046"`. Проверка
на равенство объявила бы такой робот устаревшим.

`server_build_differs` — обратное значение, и это вывод, а не факт из контракта:
api.md описывает `rv` и `sv`, но нигде не обещает, что ближайший рестарт применит
`sv`. Читать его следует как «на сервере лежит не та сборка, что запущена»,
а не как обещание про следующий старт.

`mc` — счётчик главного цикла на момент снимка. Сам по себе он ничего не говорит
о перезапусках: сравнивать его можно только с ранее записанным значением.
